...
public class BoardChip extends JPanel{
	...
	public int setChip(Ownership owner) {
		if(this.owner != Ownership.NONE) return -1;
		this.owner = owner;
		if(owner == Ownership.AI) this.setBackground(new Color(171,122,101)); // brown
		return 0;
	}
	
	/** Funcion que termina el juego y manda un mensaje con el resultado de la partida.
	 * @param value Valor de la partida. Si la IA gano, entonces value=-1, si el usuario
	 * gano, entonces 1, y si hubo empate, 0. */
	private void endGame(int value) {
		String message = (value == 0)? "Hubo empate :/" : null;
		JOptionPane.showMessageDialog(null, (message == null)? ((value == -1)? "Has ganado :)" : "Has perdido :(") : message);
		System.exit(0);
	}
}